...
static void button_task(void *arg) {
  ...
  for(;;) {
    ...
    if(1) {
      ...
      if(buttonState.event == ButtonEvent::LongPress) {
        if(!once) {
          xEventGroupSetBits(device_mode_event_group, USING_OTA_UPLOAD);
          Serial.println("OTA Upload Mode Enabled");
          once = true;
        } else {
          xEventGroupClearBits(device_mode_event_group, USING_OTA_UPLOAD);
          Serial.println("OTA Upload Mode Disabled");
          once = false;
        }
      }
